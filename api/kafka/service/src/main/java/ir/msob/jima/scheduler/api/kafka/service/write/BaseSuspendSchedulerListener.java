...
public interface BaseSuspendSchedulerListener<
    ...
    Logger log = LoggerFactory.getLogger(BaseSuspendSchedulerListener.class);

    @PostConstruct
    default void suspend() {
        String operation = Operations.SUSPEND;

        ContainerProperties containerProperties = createContainerProperties(operation);
        containerProperties.setMessageListener((MessageListener<String, String>) dto -> serviceSuspend(dto.value()));
    ...
    @SneakyThrows
    @CallbackError("dto")
    private void serviceSuspend(String dto) {
        ...
        getService().suspend(message.getData().getCriteria(), user)
                .subscribe(suspend -> sendCallbackBoolean(message, suspend, OperationsStatus.SUSPEND, user));
    }
}
