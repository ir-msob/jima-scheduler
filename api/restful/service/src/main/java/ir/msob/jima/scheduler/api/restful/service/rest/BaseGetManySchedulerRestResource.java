...
public interface BaseGetManySchedulerRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseGetManySchedulerRestResource.class);
    ...
    @MethodStats
    default ResponseEntity<Mono<Collection<JobDto>>> getMany(JobCriteria criteria, ServerWebExchange serverWebExchange, Principal principal) throws JsonProcessingException {
        log.debug("REST request to getMany job, criteria {}", criteria);
        Optional<USER> user = getUser(serverWebExchange, principal);
        return this.getManyResponse(this.getService().getMany(criteria, user), user);

    }

    default ResponseEntity<Mono<Collection<JobDto>>> getManyResponse(Mono<Collection<JobDto>> result, Optional<USER> user) {
