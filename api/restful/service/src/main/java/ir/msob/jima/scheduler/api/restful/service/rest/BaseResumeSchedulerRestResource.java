...
public interface BaseResumeSchedulerRestResource<
    ...
    Logger log = LoggerFactory.getLogger(BaseResumeSchedulerRestResource.class);

    @GetMapping(Operations.RESUME)
    @ApiResponses(value = {@ApiResponse(code = 200, message = "Return a domain or null"),
            @ApiResponse(code = 400, message = "If the validation operation is incorrect throws BadRequestException otherwise nothing", response = BadRequestResponse.class)})
    @MethodStats
    default ResponseEntity<Mono<Boolean>> resume(JobCriteria criteria, ServerWebExchange serverWebExchange, Principal principal) throws JsonProcessingException {
        log.debug("REST request to resume job, criteria {}", criteria);
    ...
    default ResponseEntity<Mono<Boolean>> resumeResponse(Mono<Boolean> result, Optional<USER> user) {
